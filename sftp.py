...
def main():
    ...
    if not st.session_state['license_validated'] and validation_key == correct_key:
        start_date = datetime.now(JAKARTA_TZ)
        with open(license_file, 'w') as file:
            file.write(start_date.isoformat())
        st.session_state['license_validated'] = True
        st.success("License key is correct. License has been validated.")
    elif not st.session_state['license_validated'] and validation_key:
        st.error("Invalid license key. Please enter the correct key to proceed.")
