...
def generate_metadata(model, img):
    caption = model.generate_content(["Generate a title in English with a length of up to 12 words, that are relevant to the image to identify the main elements (subject, activity, context), create a clear and concise description that includes these elements, use relevant keywords for SEO optimization, avoid using brand names or copyrighted elements in the title, and craft a short, precise, and descriptive title to ensure readability and ease of search.", img])
    tags = model.generate_content(["Generate up to 45 keywords in English that are relevant to the image (Ensure each keyword is a single word, separated by commas).", img])
